...
elif [[ $MODEL_TAG == codet5_small_finetuned ]]; then
  MODEL_TYPE=codet5
  TOKENIZER=roberta-base
  MODEL_PATH=${WORKDIR}/pretrained_models/codet5_small_finetuned
...
CUDA_VISIBLE_DEVICES=${GPU} \
  ...
  --data_dir /mnt/data  --cache_path ${CACHE_DIR} --res_dir ${RES_DIR} --res_fn ${RES_FN} \
