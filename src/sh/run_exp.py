...
def get_args_by_task_model(task, sub_task, model_tag):
    if task == 'translate':
        ...
        if sub_task == 'javascript':
# 11/02/2021 15:20:43 - INFO - utils -   Read 58025 examples, avg src len: 11, avg trg len: 114, max src len: 165, max trg len: 512
# 11/02/2021 15:20:43 - INFO - utils -   [TOKENIZE] avg src len: 12, avg trg len: 136, max src len: 177, max trg len: 3016
            src_len = 170  
            trg_len = 256 
            epoch = 30
            patience = 3
...
def get_sub_tasks(task):
    if task == 'summarize':
        sub_tasks = ['ruby', 'javascript', 'go', 'python', 'java', 'php', 'sn_javascript']
    elif task == 'translate':
        sub_tasks = ['java-cs', 'cs-java']
    elif task == 'refine':
        sub_tasks = ['small', 'medium']
    elif task in ['concode', 'defect', 'clone']:
        sub_tasks = ['java', 'javascript']
    return sub_tasks
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_tag", type=str, default='codet5_small',
                        choices=['roberta', 'codebert', 'codet5_small', 'codet5_base', 'codet5_small_finetuned'])
